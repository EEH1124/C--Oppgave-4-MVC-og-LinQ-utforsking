...
// 2️ Defines a class to represent a Digimon
...
class Program
{
    static void Main()
    {
        // 4️ Defines the CSV file location
        string filePath = "DigiDB_digimonlist.csv"; // Ensure this file is in the project folder

        // 5️ Checks if the file exists before proceeding
        if (!File.Exists(filePath))
        {
            Console.WriteLine("Error: Digimon database file not found.");
            return; // Stops the  execution if the file is missing
        }

        // 6️ Load Digimon data from the CSV file into a list
        ...
        {
            // 7️ Ask the user to enter a Digimon name
            ...
            // 8️ Checks if the user wants to exit
    ...
    // 1️1️ Function to load the Digimon data from the CSV file
    static List<Digimon> LoadDigimonData(string filePath)
    {
        using (var reader = new StreamReader(filePath)) // Opens the file for reading
